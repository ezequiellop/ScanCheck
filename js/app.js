...
function saveJiraConfig() {
  const cfg={domain:'danaide-enterprise.atlassian.net',email:document.getElementById('jira-email').value.trim(),token:document.getElementById('jira-token').value.trim(),project:document.getElementById('jira-project').value.trim().toUpperCase(),issueType:document.getElementById('jira-issuetype').value.trim()};
...
async function detectJiraIssueTypes() {
  const email = document.getElementById('jira-email').value.trim();
  const token = document.getElementById('jira-token').value.trim();
  const project = document.getElementById('jira-project').value.trim().toUpperCase();
  const resultsEl = document.getElementById('jira-issuetype-results');
  if (!email || !token || !project) {
    showToast('Completá email, token y proyecto primero', 'error');
    return;
  }
  resultsEl.textContent = 'Consultando...';
  try {
    const r = await fetch(JIRA_PROXY_URL, {
      method: 'POST',
      headers: {'Content-Type':'application/json'},
      body: JSON.stringify({
        jiraDomain: 'danaide-enterprise.atlassian.net',
        email, apiToken: token,
    ...
    if (types.length === 0) {
      resultsEl.textContent = 'No se encontraron tipos. Verificá la clave del proyecto.';
      return;
    }
    resultsEl.innerHTML = 'Tipos disponibles: ' + types.map(t =>
      `<span onclick="document.getElementById('jira-issuetype').value='${t}'" style="cursor:pointer;text-decoration:underline;color:var(--accent);margin-right:8px">${t}</span>`
    ).join('');
  } catch(e) {
    resultsEl.textContent = 'Error al consultar: ' + e.message;
  }
}
window.detectJiraIssueTypes = detectJiraIssueTypes;
    ...
    for(const s of scans){
      const sr = await jiraCall('/rest/api/3/issue', {
        fields:{project:{key:cfg.project},summary:`[${opLabel(s.opType)}] Puesto ${s.puesto} — Serie ${s.serie} (Ref: ${parentKey})`,
          ...
          issuetype:{name:cfg.issueType||'Incidente'}}
      });
      if(sr.ok){
