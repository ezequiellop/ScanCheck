...
async function startQRScan() {
  try {
    ...
    vid.srcObject=qrStream; vid.classList.remove('hidden');
    document.getElementById('qr-controls').classList.remove('hidden');
    qrScanning=true; scanQRFrame(vid);
  } catch(e) { showToast('No se pudo acceder a la cámara','error'); }
}
...
function scanQRFrame(vid) {
  ...
  if (vid.readyState===vid.HAVE_ENOUGH_DATA && typeof jsQR!=='undefined') {
    ...
    if (code) { processQRData(code.data); stopQRScan(); return; }
  }
