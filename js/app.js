...
async function viewReport(id) {
  ...
  if (!rep) return;
  viewingReportId = id;
  // Auto-sync to Firebase if not yet saved there
  if (!rep.fbId && currentUser) {
    fbSaveReport(rep).then(fbId => {
      const idx2 = localReports.findIndex(r=>r.id===id);
      if (idx2>=0) localReports[idx2].fbId = fbId;
...
async function syncAllReports() {
  ...
  showToast(`Sincronizando ${unsynced.length} informes...`);
  let ok = 0;
  for (const rep of unsynced) {
    try {
      ...
      ok++;
    } catch(e) { console.warn('Sync failed:', rep.id, e.message); }
  }
  renderHistory();
